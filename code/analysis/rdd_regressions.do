...
replace capacity_factor = capacity_factor + frac_marginal*capacity_factor/100 if flag_1603 == 1 & frac_marginal != .
replace capacity_factor = mcf if capacity_factor > mcf

eststo clear

eststo: ivreg2 $dv (flag_1603 = policy) $avs $covars , cluster(facilityid) partial($avs ) first
	mat def a = e(first)
	estadd local fstat = round(a[4,1])
	estadd local regtype "2SLS", replace
	estadd local state "N", replace
	estadd local covars "Y", replace
	estadd local r2a = "-", replace
estimates save "$outdir/estimates/rd_robust_fracLMP", replace

eststo: ivreg2 $dv (flag_1603 = policy) $avs $covars _Dst* , cluster(facilityid) partial($avs _Dst*) first
	mat def a = e(first)
	estadd local fstat = round(a[4,1])
	estadd local regtype "2SLS", replace
	estadd local state "Y", replace
	estadd local covars "Y", replace
	estadd local r2a = "-", replace
estimates save "$outdir/estimates/rd_robust_fracLMP_st", replace
	...
	foreach spec in nostateFEs stateFEs {
		...
		if `var'==log_netgen {
			coefplot ///
				...
				note(Regression specifications correspond to preferred model in text. Spikes span 95% confidence intervals.)
			...
			coefplot ///
				...
				coeflabels(flag_1603 = " ") ///
				xtitle("Bandwidth (Months)") ///
				ytitle("Coefficient Estimates") ylabel(#6) ///
				note(Regression specifications correspond to preferred model in text. Spikes span 95% confidence intervals.)
