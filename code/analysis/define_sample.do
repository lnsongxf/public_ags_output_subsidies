...
*DROP 1603 PLANTS WHERE THERE IS AMBIGUITY ABOUT MATCH TO EIA. SEE NOTES/ APPENDIX 
...
*BRING IN NEGATIVE PRICE SHARE OF NEAREST NODE
use $repodir/generated_data/negative_prices_by_facility.dta, clear
gen flag_in_lmp = 1
replace flag_in_lmp = 0 if iso_match == 0 & km_to_my_node > 100
egen avgFrac0 = mean(frac0), by(facilityid)
save lmpdat, replace
...
merge 1:1 facilityid year month using lmpdat, ///
	keep(match master) nogen
replace flag_in_lmp = 0 if flag_in_lmp == .
